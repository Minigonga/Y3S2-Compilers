...
public class OptimizationsTest {
    ...
    static OllirResult getOllirResultOptExtra(String filename) {
        Map<String, String> config = new HashMap<>();
        config.put(ConfigOptions.getOptimize(), "true");
        config.put(ConfigOptions.getExtra(), "true");
    ...
    @Test
    public void constPropSimpleExtra() {

        String filename = "const_prop_fold/PropSimple.jmm";

        OllirResult original = getOllirResult(filename);
        OllirResult optimized = getOllirResultOptExtra(filename);
